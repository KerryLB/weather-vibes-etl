...
### 21. forecast_fits Star Schema PK and FK Columns

**What changed:**
- `forecast_fits.csv` SQL query updated to include three key columns:
  - `forecast_fits_id` — surrogate primary key aliased from `weather_forecast.forecast_id`
  - `vibe_id` — foreign key aliased from `vibe_dimension.vibe_id`
  - `fit_id` — foreign key aliased from `fit_recommendations.fit_id`
- `export_to_csv()` docstring updated to document all 5 output files and clarify which 3 are for Power BI
- Final log message updated to confirm PK and FK columns are present

**Why:**
- Previously `forecast_fits` had no explicit key columns — Power BI was joining on `weather_vibe` text matching which is fragile
- Adding explicit integer PK and FK columns makes the star schema proper and unambiguous
- Power BI relationships can now be built on `vibe_id` and `fit_id` instead of text fields
- `weather_forecast.csv` and `forecast_vibe.csv` are kept for DB documentation but are no longer needed in Power BI

---

### 22. Data Directory (data/) Structure

**What changed:**
- All pipeline outputs — database, CSVs, and cache — now written to a `data/` subdirectory
- `DATA_DIR = "data"` constant added at the top of the script
- `DB_PATH` uses `DATA_DIR` for centralized path management
- Cache stored at `data/.cache` instead of project root
- `.gitignore` updated to exclude `data/` entirely

**Why:**
- Keeps the project root clean — only the script, README, requirements, and docs live there
- One `.gitignore` line covers all generated outputs
- Follows standard data engineering project conventions
- Power BI points to a stable `data/` folder path

---

### 23. ERD Rebuilt as Two-Section Diagram

**What changed:**
- `Weather_Vibes_ERD.html` rebuilt with two clearly separated sections:
  - Section 1: Normalized database schema showing all 4 tables in `weather.db` with PK/FK indicators, column names, data types, and one-to-many relationship lines
  - Section 2: Power BI star schema showing only the 3 tables used in the dashboard with `forecast_fits` as the center fact table and explicit `vibe_id` and `fit_id` FK relationships
- `Weather_Vibes_ERD.pdf` added as a PDF version for BRD embedding
- A note at the top of Section 2 explicitly states which 3 CSV files Power BI imports

**Why:**
- Previous ERD was a single messy diagram mixing normalized and analytics layers
- Two-section layout makes each schema tell its own clean story
- Eliminates the cluttered bridge table relationships from the Power BI view

---

### 24. README Fully Rewritten

**What changed:**
- Project structure updated to show `data/` directory and `docs/` folder
- Schema design section updated to explain the two-layer approach with PK/FK details on `forecast_fits`
- Database schema section split into Normalized Layer and Analytics Schema (Power BI Star Schema)
- New CSV Export Reference table added clarifying which files are for Power BI and which are for DB documentation
- Power BI relationships updated to use `vibe_id` and `fit_id` FK columns instead of text matching
- Documentation artifacts section updated with PDF versions
...
| 33 | forecast_fits PK and FK columns | No key columns | Added forecast_fits_id PK, vibe_id FK, fit_id FK to SQL export query |
| 34 | data/ directory | Files written to root | All outputs now written to data/ subdirectory via DATA_DIR constant |
| 35 | ERD | Single messy diagram | Rebuilt as two-section diagram — normalized DB and Power BI star schema |
| 36 | README | Missing data/ folder, no CSV reference table | Fully rewritten with updated structure, schema tables, and CSV reference |
